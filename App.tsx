...
const AnimatedBackground = () => {
  ...
  React.useEffect(() => {
    ...
    const perform = () => {
      ...
      const pad = 12; // leave a margin so particles can appear around elements
  ...
  // Generate particles (mixed hearts and brains) — lower intensity / background (memoized)
  const particles = React.useMemo(() => {
    const particleCount = 20; // increased slightly for more presence
    return Array.from({ length: particleCount }).map((_, i) => {
      ...
      const size = type === 'brain' ? Math.round(baseSize * 1.2) : baseSize; // brains ~20% larger
      return {
        id: i,
        type,
        left,
  ...
  return (
    <>
      ...
      <div aria-hidden className={`fixed inset-0 -z-20 ${useMotion ? '' : 'reduced'}`}>
        ...
        {/* Overlay gradient to make motion visible across solid sections */}
        <div
        ...
        {/* Soft blobs for depth (keep a couple so it still looks nice) */}
      ...
      {/* Particle layer: subtle background (slightly above content so it shows through) */}
...
const Navbar = () => {
  ...
  return (
    <nav className={`no-particles fixed w-full z-50 transition-all duration-500 ${isScrolled ? 'py-4 glass shadow-sm' : 'py-6 bg-transparent'}`}>
